...
for item in ${array[*]}
do
	printf $Msys2Path'\\'$item"\t to "$ReleasePath"\n"
    cp $Msys2Path'\\'$item $ReleasePath
done

echo ''
echo 'Deploying QtDLLs via windeployqt.exe'
qtDeployPath=' --release --qmldir '$sourcePath' '$ReleasePath
echo 'Loading please wait!'
exec $Msys2Path'\\'windeployqt.exe $qtDeployPath

echo ''
echo 'Deploying Steam SDK'
cp $sourcePath'/'$SteamSDK'steam_api64.dll' $ReleasePath
cp $sourcePath'/'$SteamSDK'steam_api64.lib' $ReleasePath
cp $sourcePath'/steam_appid.txt' $ReleasePath
