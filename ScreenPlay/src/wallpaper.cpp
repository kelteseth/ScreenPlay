...
//    SetLayeredWindowAttributes(m_worker_hwnd, RGB(0, 0, 0), 0, LWA_ALPHA);
    ...
    int xProxy = m_monitor.m_geometry.x();
    int yProxy = m_monitor.m_geometry.y();
    int widthProxy = m_monitor.m_geometry.width();
    int heightProxy = m_monitor.m_geometry.height();
    this->setWidth(widthProxy);
    this->setHeight(heightProxy);
    this->setX(xProxy);
    this->setY(yProxy);
    qDebug() << "CreateWindow" << xProxy << yProxy << widthProxy << heightProxy;
    ...
    m_quickRenderer.data()->setGeometry(m_monitor.m_geometry);
